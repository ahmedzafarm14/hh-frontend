...
export default function Chats() {
  ...
  const rooms = useMemo(() => {
    ...
    return d?.data?.rooms || d?.rooms || d?.data || [];
  }, [roomsResp]);

  const messages = useMemo(() => {
    const d = messagesResp;
    // API docs: { success: true, data: { messages: [...] } }
    const apiMessages = d?.data?.messages || d?.messages || d?.data || [];
  ...
  const apiBaseUrl =
    import.meta.env.VITE_CHAT_BACKEND_URL || "http://localhost:3002";
  ...
  const roomIdOf = (room) =>
    room?._id || room?.id || room?.roomId || room?.room_id || null;
  ...
  useEffect(() => {
    ...
    const room = rooms.find(
      (r) => String(roomIdOf(r)) === String(targetRoomId)
    );
    if (room) {
  ...
  const handleSendMessage = async (e) => {
    ...
    if (!socketConnected) {
      console.warn(
        "Socket not connected, message may not be delivered in real-time"
      );
    }
  ...
  return (
    <div
      className={`flex h-[calc(100vh-80px)] min-h-0 gap-2 ${
        userRole === "owner" ? "" : "pt-20"
      }`}
    >
      {/* Left Sidebar */}
      <div
          ...
          {roomsLoading || roomsFetching ? (
            ...
            rooms
              ...
              .map((room) => {
                ...
                const lastText =
                  last?.message || last?.text || last?.content || "";
                const lastAt = last?.createdAt
                  ? dayjs(last.createdAt).format("ddd, h:mm A")
                  : "";
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-4 bg-LightBackground">
              {messagesFetching ? (
                ...
                messages.map((m) => {
                  ...
                  const senderIdStr =
                    senderIdRaw != null ? String(senderIdRaw) : "";
                  const isExplicitMine =
                    m?.isMe ||
                    m?.isMine ||
                  ...
                  const mine = Boolean(
                    isExplicitMine ||
                      (meIdStr && senderIdStr && senderIdStr === meIdStr)
                  );

                  const text = m?.message || m?.text || m?.content || "";
                  const created = m?.createdAt
                    ? dayjs(m.createdAt).format("ddd, h:mm A")
                    : "";
                  return (
                    <div
                      key={m?._id || m?.id}
                    ...
                    >
                      <div
                        className={`max-w-xs rounded-2xl p-3 ${
                          mine ? "bg-blue-500 text-white" : "bg-white"
                        }`}
                      >
